...
class FourSequenceReadSet:
    ...
    def assignRead(self, sTable, replace_id=True):
        ...
        if replace_id:
            self.sample = sTable.getSampleID(self.sample,self.primer[0])
...
class OneSequenceReadSet:
    ...
    def getRead(self):
        ...
        """
        if self.primer_ID != None:
            read1_name = "%s 1:N:0:%s:%s" % (self.name, self.sample_ID, self.primer_ID)
        else:
            read1_name = "%s 1:N:0:%s" % (self.name, self.sample_ID)
        r1 = '\n'.join([read1_name, self.read_1,'+',self.qual_1])
        return [r1]
