...
P7_barcodeRC <- as.character(reverseComplement(DNAStringSet(barcodes$Seq[P7idx])))
...
lookupTable <- data.frame(Barcode = paste(rep(barcodeIdentifier, each=48), 1:(length(barcodeIdentifier)*48), sep=""),
                          ...
                          P7_barcode=rep(P7_barcodeRC[1:(length(barcodeIdentifier)*2)], each=24))
